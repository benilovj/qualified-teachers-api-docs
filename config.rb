...
spec_src_url = "https://raw.githubusercontent.com/DFE-Digital/qualified-teachers-api/main/docs/api-specs/%s.json"
jq_cmd = %s{jq 'walk(if type == "object" and has("application/*+json") then del(."application/*+json") else . end)'}

system "curl #{spec_src_url % 'v1'} | #{jq_cmd} > tmp/v1.json"
system "curl #{spec_src_url % 'v2'} | #{jq_cmd} > tmp/v2.json"
